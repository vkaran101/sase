...
class Admin_auth extends CI_Controller {
  ...
  function index()
  {
    ...
    {
      //render error because they must be an administrator to view this
  ...
  {
    $this->form_validation->set_rules('username','Username','required|alpha_dash|callback_username_check');
    if ($this->form_validation->run() == false)
    {
    ...
    {
      // get identity from username
      $identity = $this->ion_auth->where('username',strtolower($this->input->post('username')))->users()->row();

      if (empty($identity))
      {
        $this->ion_auth->set_message('No record of username found.');
        $this->session->set_flashdata('message',$this->ion_auth->messages());
  ...
  // check that the username is in database
  ...
  function deactivate($id = NULL)
  {
    ...
    else
    {
      ...
      if ($this->input->post('confirm') == 'yes')
      {
        ...
        {
          show_error('This form submit did not pass our security checks.');
        }
  ...
  function create_user()
  {
    ...
    $this->form_validation->set_rules('username','Username',
      'required|alpha_dash|is_unique['.$tables['users'].'.username]'
    );
    ...
    if ($this->form_validation->run() == true && $this->ion_auth->register($username,$pwd,$email,$optionals))
    {
    ...
    else
    {
      ...
      $errors = ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message'));
  ...
  function edit_user($id)
  {
    ...
    $this->form_validation->set_rules('last_name','Last Name','required|alpha|xss_clean');
    $this->form_validation->set_rules('email','Email',
      'required|valid_email|callback_email_check['.$user->id.']'
    );
  ...
  function email_check($str,$id)
  {
    ...
    foreach ($users as $user)
    {
      if ($user->id != $id && $user->email == $str) {
        $this->form_validation->set_message('email_check','The %s field must contan a unique value.');
        return FALSE;
      }
    }
    return TRUE;
  ...
  {
    // bail if no group id given, not logged in, or not admin
    if (!$id || empty($id) || !$this->ion_auth->logged_in() || !$this->ion_auth->is_admin())
    {
      redirect('/admin/auth','refresh');
    }
